...
def load_model(model_name):
    ...
    try:
        ...
        if WhisperModels[model_name] is None:
            ...
            WhisperModels[model_name] = whisper.load_model(ModelNames[model_name].value, device = DEVICE)
            #
...
def handle_file_upload(request):
    ...
    if 'file' in request.files:
        ...
        print("file in request.files")
        file_input = request.files['audio']
        if file_input.filename == '':
            return None, jsonify({'error': 'No file selected for uploading'}), 400
        file_path = os.path.join('temp_audio', file_input.filename)
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
...
if __name__ == "__main__":
    ...
    def transcribe():
        ...
        if request.is_json and 'model' in request.json:
            model_name = request.json.get('model', 'large_v3_turbo')
        else:
            model_name = request.form.get('model', 'large_v3_turbo')
